...
if [[ -z "${RUNFAST}" ]]; then   
    ...
    echo "Running SLOW"
    Rscript -e "setwd('/webrender_intel_win10_nightly');library(rmarkdown); render('driver.Rmd',param=list(args=list(use_email=Sys.getenv('USE_EMAIL'))));render('dashboard.Rmd',output_dir='/tmp/output/')"
