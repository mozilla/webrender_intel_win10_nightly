...
then
    ...
    Rscript -e "setwd('/webrender_intel_win10_nightly'); source('query.R');g=bq(use_email='no'); g$q('select client_id from telemetry.main where date(submission_timestamp)=\'2020-01-01\' limit 10')"
    bq query --use_legacy_sql=false "select client_id from telemetry.main where date(submission_timestamp)='2020-01-01'"
