...
if (is.null(args$max.build.id)) {
    ...
    max.build.id <- as.Date(args$max.build.id, "%Y%m%d")
}
...
Mean <- mean
active.hours.cmp <- compare.usage[
    active_hours < quantile(active_hours, 0.999),
    mAndCi(.SD[, list(id, branch, x = active_hours)], "x", REPLICATES, oper = Mean),
    by = buildid
][, label := "active_hours"][order(buildid, what), ]

total.hours.cmp <- compare.usage[
    total_hours < quantile(total_hours, 0.999),
    mAndCi(.SD[, list(id, branch, x = total_hours)], "x", REPLICATES, oper = Mean),
    by = buildid
][, label := "total_hours"][order(buildid, what), ]
total.urisperhr.cmp <- compare.usage[
    total_uri / (1 / 3600 + active_hours) < quantile(total_uri / (1 / 3600 + active_hours), 0.999),
    mAndCi(.SD[, list(id, branch, x = total_uri / (1 / 3600 + active_hours))], "x", REPLICATES, oper = Mean),
    by = buildid
][, label := "uri_per_profile_activehour"][order(buildid, what), ]


ncrash.all.cmp <- compare.usage[
    ncrash_all / (1 / 3600 + total_hours) < quantile(ncrash_all / (1 / 3600 + total_hours), 0.999),
    mAndCi(.SD[, list(id, branch, x = 1000 * ncrash_all / (1 / 3600 + total_hours))], "x", REPLICATES, oper = Mean),
    by = buildid
][, label := "ncrash_all_perkhr"][order(buildid, what), ]

ncrash.content.cmp <- compare.usage[
    ncrash_content / (1 / 3600 + total_hours) < quantile(ncrash_content / (1 / 3600 + total_hours), 0.999),
    mAndCi(.SD[, list(id, branch, x = ncrash_content / (1 / 3600 + total_hours))], "x", REPLICATES, oper = mean),
    by = buildid
][, label := "ncrash_content_perkhr"][order(buildid, what), ]
ncrash.content.cmp <- ncrash.content.cmp [, ":="(est = est * 1000, low = low * 1000, high = high * 1000)]

ncrash.main.cmp <- compare.usage[
    ncrash_main / (1 / 3600 + total_hours) < quantile(ncrash_main / (1 / 3600 + total_hours), 0.999),
    mAndCi(.SD[, list(id, branch, x = 1000 * ncrash_main / (1 / 3600 + total_hours))], "x", REPLICATES, oper = Mean),
    by = buildid
][, label := "ncrash_main_perkhr"][order(buildid, what), ]

ncrash.shutdown.cmp <- compare.usage[
    ncrash_shutdown / (1 / 3600 + total_hours) < quantile(ncrash_shutdown / (1 / 3600 + total_hours), 0.999),
    mAndCi(.SD[, list(id, branch, x = 1000 * ncrash_shutdown / (1 / 3600 + total_hours))], "x", REPLICATES, oper = Mean),
    by = buildid
][, label := "ncrash_shutdown_perkhr"][order(buildid, what), ]

## ncrash.oom.cmp <- compare.usage[ncrash_oom/(1/3600+total_hours)<=quantile(ncrash_oom/(1/3600+total_hours),1),
...
ncrash.gpu.cmp <- compare.usage[
    ncrash_gpu / (1 / 3600 + total_hours) < quantile(ncrash_gpu / (1 / 3600 + total_hours), 1),
    mAndCi(.SD[, list(id, branch, x = 1000 * ncrash_gpu / (1 / 3600 + total_hours))], "x", REPLICATES, oper = Mean),
    by = buildid
][, label := "ncrash_gpu_perkhr"][order(buildid, what), ]


device.reset.cmp <- compare.usage[
    device_resets / (1 / 3600 + total_hours) < quantile(device_resets / (1 / 3600 + total_hours), 0.999),
    mAndCi(.SD[, list(id, branch, x = device_resets / (1 / 3600 + total_hours))], "x", REPLICATES, oper = Emean),
    by = buildid
][, label := "crash_device_resets_perkhr"][order(buildid, what), ]
device.reset.cmp <- device.reset.cmp [, ":="(est = est * 1000, low = low * 1000, high = high * 1000)]

checker.cmp <- compare.usage[
    checkerboard_severe_events / (1 / 3600 + total_hours) < quantile(checkerboard_severe_events / (1 / 3600 + total_hours), 0.999),
    mAndCi(.SD[, list(id, branch, x = checkerboard_severe_events / (1 / 3600 + total_hours))], "x", REPLICATES, oper = Emean),
    by = buildid
][, label := "severe_checkerboard_perkhr"][order(buildid, what), ]
...
```{r fcontent_frame_time_vsync,eval=TRUE}
...
```{r f9,eval=if(is.debug) FALSE else TRUE}
...
l <- glue(
    "bq load --noreplace   --project_id='{env_vars[[\'project\']]}'  --source_format=CSV --skip_leading_rows=1 --null_marker=NA",
