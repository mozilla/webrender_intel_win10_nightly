...
histogram.bin.errs <- function(histo, REP = 5000) {
    z <- histo[,
        ...
        by = list(buildid = buildid, branch)
    ][order(buildid, branch), ]
...
histogram.summary.cuts <- function(histo, REP = 5000, CUT) {
    ...
    br <- histo[,
        ...
        by = list(buildid = buildid, what = branch)
    ][order(buildid, what), ]
    ...
    u3 <- list(
        ...
        layer = list(
            list(
                ...
                encoding = list(
                    x = list(
                        field = "buildid2", type = "temporal", timeUnit = "yearmonthdatehoursminutes",
                        axis = list(
                    ...
                    tooltip = list(
                        list(field = "buildid", type = "nominal"),
...
create.figure <- function(LL, title, width = NULL, height = NULL, yaxislab = "Estimate", bqtable, g,
    ...
    xx <- w[, list(
        UsingDataTill = date_computed, buildid = parse_date(buildid),
        Branch = what,
